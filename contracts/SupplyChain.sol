...
contract SupplyChain {
  ...
  // <modifier: isOwner
   ...
   modifier isOwner(address _address) {
      require(msg.sender == _address, 'You are not owner');
      _;
  }
  ...
  modifier forSale(uint _sku) {
      ...
      require(items[_sku].price != 0, 'item is not for sale, no price');
      _;
  }
  ...
  checkValue(sku)
  {
      bool sent = items[sku].seller.send(items[sku].price);
      require(sent, 'failed to send money to seller');
  ...
  sold(sku)
  {
    require (msg.sender == items[sku].seller, 'just seller can ship'); 
  ...
  shipped(sku)
  {
    require (msg.sender == items[sku].buyer, 'just buye can call receiveItem'); 
